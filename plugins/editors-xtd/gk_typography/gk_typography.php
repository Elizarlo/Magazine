...
class plgButtonGK_Typography extends JPlugin {
    ...
    function onDisplay($name) {
    	if(JRequest::getCmd('option') == 'com_content' || JRequest::getCmd('option') == 'com_k2') {
			...
			// set modal to false to stop using mce modal
	        $button->set('modal', false);
	        
	        $button->set('class', 'btn btn-info');

			// set link to # and using SqueezeBox
			$button->set('link', '#');
			$button->set('onclick', ' SqueezeBox.setContent("clone", jQuery(".gk_typography_content")); ');

	        $button->set('text', JText::_('GK Typography'));

	        $button->set('name', 'pencil-2 gk-typo');

// 	        $button->set('options', "{handler:'clone',target:$$('.gk_typography_content')[0],size:{x:400,y:400}}");

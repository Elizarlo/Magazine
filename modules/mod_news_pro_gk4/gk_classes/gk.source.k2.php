	...
	function getSources($config) {
		...
		$query_name = '
			...
			LEFT JOIN 
				...
				ON 
				c.id = content.catid AND content.language IN ("*", "'.$currentTag.'") 
             '.$tag_join.' 
			WHERE 
				1=1 
				'.$where.'
		...
		if(count($categories) > 0){
			...
			WHERE 
				content.trash = 0'.(($config['unauthorized'] == 0) ? ' 
					...
					AND content.language IN ("*", "'.$currentTag.'") 
			...
			FROM 
				...
				LEFT JOIN 
					...
					ON cats.id = content.catid AND cats.language IN ("*", "'.$currentTag.'") 
				LEFT JOIN 
					#__users AS users 
